...
const createGroup = (userId, date, tasks, height, onClickTaskCheckbox) => (<div key={userId + '|' + date} style={STYLE_GROUP(height)}>
    <header style={DATE_STYLE}>{moment(date).format('YYYY-MM-DD # dddd')}</header>
    { tasks.map(t => <li>
                        <label style={t.done ? { textDecoration: 'line-through'} : null}>
